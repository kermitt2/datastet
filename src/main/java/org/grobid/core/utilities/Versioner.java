...
public class Versioner {
    ...
    private static String readFromSystemPropertyOrFromFile(String systemPropertyName, String filePath) {
        String value = UNKNOWN_VERSION_STR;
        ...
        if (systemPropertyValue != null) {
            ...
            try (InputStream is = GrobidProperties.class.getResourceAsStream(filePath)) {
                if (is != null) {
                    String tmp = IOUtils.toString(is, StandardCharsets.UTF_8);
                    if (!StringUtils.startsWithIgnoreCase(tmp, "${project_")) {
                        value = tmp.trim();
                    }
                }
            } catch (IOException e) {
                LOGGER.error("Cannot read {} from resources", filePath, e);
            }
        }
        return value;
    }
